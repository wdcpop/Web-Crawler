...
class MyResultHandler(BasicResultHandler):
    def __init__(self, spider_mname, to_save):
        ...
        self.spider_title = to_save.get('config', {}).get('title', 'NONAME')
        ...
        self.items_table = self.content_db['items']
        self.sources_table = self.content_db['sources']
        self.err_items_table = self.content_db['err_items']
    ...
    def on_new_results(self, results, total_time, urls_count=None, crawler=None):
        ...
        # 清洗results, 每个result必需标题和内容有一个不为空  /home/wdcpop/WALL/NewsCrawler/crawler/result_handlers/my_result_handler.py
        ...
        try:
            self.redis.publish('crawled', json.dumps(crawled))
        ...
        if len(results) > 0:
            ...
            for i in results:
                ...
                i['coops'] = crawler.single_config.get('coops')

                if not i.get('title'):
                    i['title'] = 'NO TITLE'

                i['source'] = self.spider_mname
                i['name'] = self.spider_title if not i.get('source_alter') else i.get('source_alter')
                try:
                    inserted = self.items_table.insert_one(i)
                    self.sources_table.update({'machine_name': self.spider_mname}, {
                        '$set': {
                                    'latestArticleTitle': i.get('title'),
                                    'latestArticleCreated': i.get('ctime'),
                                }
                    })
                except Exception as e:
                    self.logger.exception(e)
                ...
                try:
                    self.redis.publish('news_updated', str(inserted.inserted_id))
                except Exception as e:
                    self.logger.exception(e)
